...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None):
        ...
        file_len = len(file_pd)
        for i in range(file_len):
            this_pd = file_pd.iloc[i]
            if this_pd['A'] not in self.answers2label:
                self.answers2label[this_pd['A']]=len(self.answers2label)
            this_js = {'Q':this_pd['Q'],'A':this_pd['A'],'label':self.answers2label[this_pd['A']],'index':i}
...
def evaluate(args, model, tokenizer,eval_when_training=False):
    ...
    #     ground_truth = list(valid_pd['A'])
        ...
        return mrr_score
    valid_pd = pd.read_csv(args.eval_data_file)
    ground_truth = list(valid_pd['A'])
